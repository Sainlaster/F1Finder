...
void CFramelessWindow::setResizeable(bool resizeable)
{
    ...
    if (m_bResizeable){
        ...
        //SetWindowLong(hwnd, GWL_STYLE, WS_POPUP |WS_CAPTION| WS_MAXIMIZEBOX| WS_SIZEBOX);
        DWORD style = ::GetWindowLong(hwnd, GWL_STYLE);
        ::SetWindowLong(hwnd, GWL_STYLE,style | WS_MAXIMIZEBOX |WS_THICKFRAME | WS_CAPTION);//style | WS_MAXIMIZEBOX | WS_THICKFRAME | WS_CAPTION);
...
bool CFramelessWindow::nativeEvent(const QByteArray &eventType, void *message, qintptr *result)
{
    ...
    case WM_NCCALCSIZE:
    {
        ...
        //		params.rgrc[0].top -= 1;
        //ЭТо было включено но руинило все нафиг
    ...
    case WM_NCACTIVATE:
    {
        if (msg->message == WM_NCACTIVATE)
            {
                *result = 1; // Возвращаем 1, чтобы сообщить системе, что сообщение было обработано
                return true; // Возвращаем true, чтобы окно не получало фокус
            }
        //qDebug()<<*msg; UpdateWindow((HWND)this->winId());
        //msg->wParam = TRUE;
        //DefWindowProc((HWND)this->winId(), msg->message, msg->wParam, msg->lParam);
        //return true;
        //return QMainWindow::nativeEvent(eventType, &mymsyg, result);
    }
    default: 
...
{
    getContentsMargins(left,top,right,bottom);//QMainWindow::
